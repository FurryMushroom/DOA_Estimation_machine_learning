...
import time
import  os
from network import MainNet, ExMSELoss, AnotherNet
...
def train(net, epochs,device, train_loader, test_loader, optimizer, criterion, scheduler=None):
    ...
    for e in range(LAST_EPOCH,epochs):
        ...
        loss_train /= (i + 1) # it's surprising that i is not freed
        angle_err_train /= (i + 1)
...
#             scheduler.step()
        ...
        if (e + 1) % 4 == 0 :
            torch.save(net.state_dict(), SAVE_PATH+'/normed_epoch_{}.pth'.format(e + 1))
...
if __name__ == '__main__':
    # os.environ["CUDA_VISIBLE_DEVICES"] = "0"  # use all gpus
    ...
    net.load_state_dict(torch.load(SAVE_PATH + '/normed_epoch_'+str(LAST_EPOCH)+'.pth'))
    ...
    # input_size
    optimizer = torch.optim.Adam(filter(lambda p: p.requires_grad, net.parameters()), lr=lr)  # only optimize the
    # parameters that requires grad.lambda argument_list:expression filter(function,iterable)
    ...
    train_loader, test_loader = gen_loader(batch_size, snr,norm='norm')
    net, _ = train(net=net, epochs=epochs,device=device,
                   train_loader=train_loader, test_loader=test_loader,
                   optimizer=optimizer, criterion=criterion,
                    # scheduler=scheduler
                   )
    # torch.save(net.state_dict(), SAVE_PATH+'/resnet34_model.pth')
